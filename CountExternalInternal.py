...
# usage python3 PlotExpDiverIntronless.py [options]
# -[chimp, mouse]: use chimp or mouse orthologs
...
GeneCats = ['Not', 'ExtWith', 'IntWith', 'ExtWithout', 'IntWithout']

# create lists of orthologous pairs for each gene category 
AllPairs = []
AllGenes = [NonOverlappingGenes, ExternalWithIntron, InternalWithIntron, ExternalIntronless, InternalIntronless] 
# loop over gene sets
...
def CreateAx(Columns, Rows, Position, figure, Data, XLabel, YLabel, YMax):
    ...
    # plot divergence
    ax.bar([0, 0.3, 0.6, 0.9, 1.2], Data[0], 0.2, yerr = Data[1], color = colorscheme,
    ...
    plt.xticks([0.1, 0.4, 0.7, 1, 1.3], XLabel, rotation = 0, size = LabelSize, color = 'black', ha = 'center', **FigFont)
...
ax1 = CreateAx(2, 1, 1, fig, [MeanDiverg, SEMDiverg], GeneCats, 'dN/dS', YMaxSeq)
