...
for i in range(len(GeneCats)):
    print(i, GeneCats[i], len(AADiffs[i]), len(dN[i]), len(dS[i]), len(Omega[i]), sep ='\t')
    print(i, GeneCats[i], min(AADiffs[i]), min(dN[i]), min(dS[i]), min(Omega[i]), sep ='\t')
    print(i, GeneCats[i], max(AADiffs[i]), max(dN[i]), max(dS[i]), max(Omega[i]), sep ='\t')
