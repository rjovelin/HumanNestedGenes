...
# generate gene pairs of external and internal genes when both, only one or none are disease genes for each disease type
AllPairs = []
for disease in [GAD, GWAS, Drivers, OMIM, DiseaseGenes]:
    # create list with both, external, internal and none being disease genes
    both, external, internal, none = [], [], [], []
    ...
    for pair in NestedPairs:
        ...
        if pair[0] in ExpressionProfile and pair[1] in ExpressionProfile:
            ...
            if pair[0] in disease and pair[1] in disease:
                both.append(pair)
            elif pair[0] in disease and pair[1] not in disease:
                external.append(pair)
            elif pair[0] not in disease and pair[1] in disease:
                internal.append(pair)
            elif pair[0] not in disease and pair[1] not in disease:
                none.append(pair)
    AllPairs.append([both, external, internal, none])

# make a list with disease types
DiseaseClasses = ['complex', 'GWAS', 'Drivers', 'mendelian', 'all']

# filter diseases if any pair count < 10
